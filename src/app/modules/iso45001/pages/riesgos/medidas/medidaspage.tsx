...
const Medidas: React.FC = () => {
	...
	const columns: ColumnsType<Medidas> = [
		{
			title: "Smart Index",
		...
		},
		{
			...
			render: (codigo: string, record: Medidas) => {
		...
		{
			title: "Código Factor de Riesgo",
			dataIndex: "codigoFactor",
			key: "codigoFactor",
			filterDropdown: ({
				setSelectedKeys,
				selectedKeys,
				...
				<div style={{ padding: 8 }}>
					<Input
						placeholder="Buscar código factor"
						value={selectedKeys[0]}
						onChange={(e) =>
							setSelectedKeys(e.target.value ? [e.target.value] : [])
						}
		...
		<>
			...
			<div>
				<Space style={{ marginBottom: 16 }}>
					...
					<Button icon={<PlusOutlined />}>Duplicar</Button>
